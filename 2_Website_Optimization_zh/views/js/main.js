...
var resizePizzas = function(size) {
  ...
  //将值转成百分比宽度的数值部分
  function determineWidthPercent(size) {
    switch (size) {
      case "1":
        return 25;
      case "2":
...
function updatePositions() {
  ...
  // 使用 requestAnimationFrame 来优化动画，动画绘制操作都转移进新增的 render 函数中
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  for (var i = 0; i < 31; i++) {
    ...
    // 由于更改了移动 pizza 的 css 属性为 transform 故在这里初始化 left
    // 给 pizza 增加 will-change 属性来避免图层重绘制
